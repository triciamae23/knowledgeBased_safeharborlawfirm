...
let state, page='home', filter='all', query='', spaceId=null, layout='list';
...
async function boot(){try{state=restrictWorkspace(await api('bootstrap'));page='home';spaceId=null;query='';render();const linkedId=pageIdFromHash();if(linkedId)await openDocument(linkedId,false);}catch(e){login();}}
...
function render(){document.title=`${state.settings.workspace_name} · Knowledge`;const title=page==='home'?'Home':page==='settings'?'Administration':page==='published'?'Published pages':page==='drafts'?'Unpublished pages':page==='space'?state.spaces.find(s=>s.id===spaceId)?.name:'All pages';$('#app').innerHTML=`<div class="reference-app"><button class="vsa-launcher" id="vsa-launcher" aria-controls="vsa-panel" aria-expanded="false">${icon('book')} KB Assistant</button><header class="global-header"><a class="brand" href="#" data-nav="home">${logo()}</a><nav class="global-nav" aria-label="Main navigation"><button data-nav="home" class="${page==='home'?'active':''}">Home</button><button data-nav="documents" class="${['documents','published','drafts','space','document'].includes(page)?'active':''}">Your work</button>${admin()?`<button data-nav="settings" class="${page==='settings'?'active':''}">Administration</button><button class="primary" data-action="new">Create</button>`:''}</nav><button class="global-search" id="header-search">${icon('search')}<span>Search knowledge</span></button><details class="account-menu"><summary aria-label="Account menu"><span class="avatar">${esc(state.user.name.split(' ').map(n=>n[0]).slice(0,2).join(''))}</span></summary><div class="account-dropdown"><b>${esc(state.user.name)}</b><small>${esc(state.user.email)}</small><span>${admin()?'Administrator':'Reader'}</span><button data-action="logout">${icon('logout')} Sign out</button></div></details></header><div class="library-shell"><aside class="folder-sidebar"><div class="space-identity"><span>${icon('book')}</span><div><b>${esc(state.settings.workspace_name)} workspace</b><small>Team knowledge base</small></div></div><nav class="workspace-nav" aria-label="Workspace navigation"><button data-nav="home" class="${page==='home'?'selected':''}">${icon('grid')} Home</button><button data-nav="documents" class="${page==='documents'?'selected':''}">${icon('file')} All pages</button><button data-nav="published" class="${page==='published'?'selected':''}">${icon('check')} Published</button>${admin()?`<button data-nav="drafts" class="${page==='drafts'?'selected':''}">${icon('edit')} Unpublished <small>${state.documents.filter(d=>d.status==='draft').length}</small></button>`:''}</nav><div class="folder-section-heading"><button class="folder-title" id="toggle-folder-tree" aria-expanded="true">${icon('down')} FOLDERS</button>${admin()?`<button class="icon-button" data-action="add-space" aria-label="Create folder">${icon('plus')}</button>`:''}</div><nav id="folder-tree" aria-label="Folders">${state.spaces.map(s=>`<button class="folder-link ${page==='space'&&spaceId===s.id?'selected':''}" data-space="${s.id}">${icon('chevron')}${icon('folder')}<span>${esc(s.name)}</span><small>${state.documents.filter(d=>d.space_id===s.id).length}</small></button>`).join('')}</nav><div class="sidebar-note">${icon('book')}<strong>A home for your team’s knowledge</strong><p>Find answers. Share ideas.<br>Build something great together.</p></div>${admin()?'<button class="sidebar-settings" data-nav="settings">'+icon('settings')+' Workspace settings</button>':''}</aside><div class="workspace-main"><div class="workspace-breadcrumb"><span>${esc(state.settings.workspace_name)} workspace</span>${icon('chevron')}<span>${esc(page==='document'?'Page':title)}</span></div><main id="content"></main></div></div></div>`;if(page==='home')dashboard();else if(page==='settings')settings();else if(page==='document')readDoc();else overview();bindShell();$('#header-search').onclick=async()=>{await navigate('documents');$('#search')?.focus();};$('#toggle-folder-tree').onclick=e=>{const tree=$('#folder-tree');tree.hidden=!tree.hidden;e.currentTarget.setAttribute('aria-expanded',String(!tree.hidden));};}
function bindShell(){$('#vsa-launcher').onclick=openVsa;document.querySelectorAll('[data-nav]').forEach(b=>b.onclick=e=>{e.preventDefault();navigate(b.dataset.nav);});document.querySelectorAll('[data-space]').forEach(b=>b.onclick=()=>{spaceId=+b.dataset.space;navigate('space');});document.querySelectorAll('[data-action]').forEach(b=>b.onclick=()=>action(b.dataset.action));}
...
async function navigate(p){const version=++navigationVersion;page=p;filter='all';query='';if(state&&!admin()){state={...state,documents:[]};$('#content').innerHTML='<div class="access-loading" role="status">Checking your page access…</div>';try{const data=await api('bootstrap');if(version!==navigationVersion)return;state=restrictWorkspace(data);if(page==='space'&&!state.spaces.some(f=>f.id===spaceId)){spaceId=null;page='documents';}if(page==='settings'||page==='drafts')page='documents';}catch(err){if(version===navigationVersion&&state){$('#content').innerHTML='<div class="empty">Unable to verify access. Refresh to try again.</div>';toast(err.message);}return;}}history.replaceState(null,'',location.pathname+location.search);render();}
...
async function editor(d={}){
  ...
  $('#editor').onsubmit=async e=>{e.preventDefault();$('#editor-serialized').value='::rich2::'+JSON.stringify({content:sanitizeRichHtml($('#editor-body').innerHTML),media:editorMedia,documentType:$('#editor-document-type').value});const data=Object.fromEntries(new FormData(e.target));data.status=e.submitter.value;data.id=d.id;const buttons=[...e.target.querySelectorAll('button[type="submit"]')];buttons.forEach(b=>b.disabled=true);try{const result=await api('documents',data);closeModal();state.currentDoc=result.id;page='document';await refresh();toast(data.status==='published'?'Page published':'Unpublished page saved');}catch(err){$('.page-editor .form-error').textContent=err.message;buttons.forEach(b=>b.disabled=false);}};
}
